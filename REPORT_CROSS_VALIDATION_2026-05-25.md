...
## E) ยืนยันตามบริบทธุรกิจจาก `Logic.md` (Group 1 vs Group 2)

อ้างอิงจาก `Logic.md` ระบบต้องไม่สลับบทบาท:
- **Group 1 (Cleansing/Mastering):** เตรียมข้อมูลพิกัดจริงและฐานความรู้ (SYS_TH_GEO, SCG raw, M_*).
- **Group 2 (Daily Ops/Search):** โหลดงานประจำวัน และ “เรียกใช้” พิกัดจาก Group 1 เพื่อเติม `LatLong_Actual`.

ผลกระทบของบั๊กที่พบเมื่อเทียบกับ Business Flow:
1. **Single Writer violation** ทำให้ Group 2 ไปเขียน knowledge ledger (`M_ALIAS`) โดยตรงระหว่าง ingestion → เสี่ยงชนกับ Group 1 pipeline และทำให้ dedup เพี้ยน.
2. **Duplicate `loadCachedGeoRows_`** เสี่ยงอ่าน dictionary ไม่ครบ ทำให้การทำความสะอาด/ตีความที่อยู่ใน Group 1 ผิด และ Group 2 lookup แมตช์พลาดตาม.
3. **Migration ไม่มี time guard** เสี่ยงหยุดกลางทาง ทำให้ฐาน Master ของ Group 1 ไม่สมบูรณ์ แล้ว Group 2 ดึงค่าพิกัดไม่เจอ.
4. **Row-by-row write** ทำให้ batch ช้าและ timeout ง่าย โดยเฉพาะรอบรันประจำวันของ Group 2.

ดังนั้นลำดับการแก้ต้องยึดแกน `Logic.md`: รักษาเสถียรภาพ Group 1 ก่อน แล้วค่อย optimize Group 2 search throughput.
